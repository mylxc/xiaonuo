...
import com.jackli.sys.modular.resource.mapper.SysModuleMapper;
import com.jackli.sys.modular.resource.param.module.SysModuleAddParam;
import com.jackli.sys.modular.resource.param.module.SysModuleEditParam;
import com.jackli.sys.modular.resource.param.module.SysModuleIdParam;
import com.jackli.sys.modular.resource.param.module.SysModulePageParam;
...
public class SysModuleServiceImpl extends ServiceImpl<SysModuleMapper, SysModule> implements SysModuleService {
    ...
    public void delete(List<SysModuleIdParam> sysModuleIdParamList) {
        ...
        if(ObjectUtil.isNotEmpty(sysModuleIdList)) {
            ...
            boolean exits=sysMenuService.getBaseMapper().exists(new LambdaQueryWrapper<SysMenu>()
                            .in(SysMenu::getModule,sysModuleIdList).eq(SysMenu::getDeleteFlag,0));
            if(exits)
            {
                throw new CommonException("模块下有关联子模块信息不可删除");
            }
            // 获取模块下的菜单、按钮
