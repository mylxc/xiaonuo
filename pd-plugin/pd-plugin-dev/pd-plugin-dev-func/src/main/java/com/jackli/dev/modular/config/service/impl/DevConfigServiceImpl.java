 ...
 */
package com.jackli.dev.modular.config.service.impl;
...
import com.jackli.dev.modular.config.mapper.DevConfigMapper;
import com.jackli.dev.modular.config.param.*;
...
public class DevConfigServiceImpl extends ServiceImpl<DevConfigMapper, DevConfig> implements DevConfigService {
    ...
    public Page<DevConfig> page(DevConfigPageParam devConfigPageParam) {
        ...
        // 查询部分字段
        queryWrapper.lambda().select(DevConfig::getId, DevConfig::getOtherValue,DevConfig::getConfigKey, DevConfig::getConfigValue,
    ...
    public void edit(DevConfigEditParam devConfigEditParam) {
        ...
        // 移除对应的缓存
        String key = CONFIG_CACHE_KEY + tenApi.getCurrentTenDomain() + ":" + devConfig.getConfigKey();
        ...
        if(ObjectUtil.isNotEmpty(devConfigIdList)) {
            ...
            if(ObjectUtil.isNotEmpty(devConfigList)) {
                ...
                this.listByIds(devConfigIdList).forEach(devConfig -> {
                    // 移除对应的缓存
                    commonCacheOperator.remove(CONFIG_CACHE_KEY + tenApi.getCurrentTenDomain() + ":" + devConfig.getConfigValue());
                });
                // 执行删除
    ...
    public void editBatch(List<DevConfigBatchParam> devConfigBatchParamList) {
        devConfigBatchParamList.forEach(devConfigBatchParam -> {
            ...
            // 移除对应的缓存
            commonCacheOperator.remove(CONFIG_CACHE_KEY + tenApi.getCurrentTenDomain() + ":" + devConfigBatchParam.getConfigValue());
        });
    }
}
