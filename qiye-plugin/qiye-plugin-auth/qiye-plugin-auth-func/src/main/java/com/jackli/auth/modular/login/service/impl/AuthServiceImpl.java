 ...
 */
package com.jackli.auth.modular.login.service.impl;
...
import com.jackli.auth.modular.login.enums.AuthDeviceTypeEnum;
import com.jackli.auth.modular.login.enums.AuthExceptionEnum;
...
public class AuthServiceImpl implements AuthService {
    ...
    public String getPhoneValidCode(AuthGetPhoneValidCodeParam authGetPhoneValidCodeParam, String type) {
        ...
        devSmsApi.sendSmsTencent("1400522364", phone, "jackli开源技术", "1502357", phoneValidCode);
...
/*        String defaultCaptchaOpen = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_CAPTCHA_OPEN_KEY);
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            ...
            List<String> ownerRoleCodes= loginUserApi.getRoleCodeListByUserId(saBaseLoginUser.getId());
            if(ObjectUtil.isNotNull(authAccountPasswordLoginParam.getRoleCode())&&!ownerRoleCodes.contains(authAccountPasswordLoginParam.getRoleCode()))
            {
                throw new CommonException(AuthExceptionEnum.VALID_ROLECODE_ERROR.getValue());
            }
            // 执行B端登录
