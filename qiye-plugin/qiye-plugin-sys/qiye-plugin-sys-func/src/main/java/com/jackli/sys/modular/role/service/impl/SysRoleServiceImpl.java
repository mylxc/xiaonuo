 ...
 */
package com.jackli.sys.modular.role.service.impl;
...
import com.jackli.auth.core.enums.SysUserStatusEnum;
/*import com.jackli.auth.core.pojo.SaBaseLoginUser;
import com.jackli.auth.core.util.StpClientLoginUserUtil;*/
import com.jackli.auth.core.util.StpLoginUserUtil;
import com.jackli.common.cache.CommonCacheOperator;
import com.jackli.common.consts.CacheConstant;
...
import com.jackli.mobile.vip.MobileMenuApi;
import com.jackli.sys.modular.org.entity.SysOrg;
import com.jackli.sys.modular.org.service.SysOrgService;
...
import com.jackli.sys.modular.relation.mapper.SysRelationMapper;
...
import com.jackli.sys.modular.role.entity.SysRole;
import com.jackli.sys.modular.role.enums.SysRoleCategoryEnum;
import com.jackli.sys.modular.role.mapper.SysRoleMapper;
import com.jackli.sys.modular.role.param.*;
import com.jackli.sys.modular.role.result.*;
import com.jackli.sys.modular.role.service.SysRoleService;
import com.jackli.sys.modular.role.vo.SysRoleVo;
import com.jackli.sys.modular.user.entity.SysUser;
import com.jackli.sys.modular.user.service.SysUserService;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public Page<SysRole> page(SysRolePageParam sysRolePageParam) {
        ...
        // 查询部分字段
        queryWrapper.lambda().select(SysRole::getId, SysRole::getOrgId, SysRole::getName,SysRole::getRoleStatus,
                SysRole::getCategory, SysRole::getSortCode);
        /********************如果登录人是超管账户才会出现超管角色*******************/
        if(!StpLoginUserUtil.getLoginUser().getAccount().equals("superAdmin"))
            queryWrapper.lambda().ne(SysRole::getName,"超管");
    ...
    @Override
    public void disableRole(SysRoleIdParam sysRoleIdParam) {
        this.update(new LambdaUpdateWrapper<SysRole>().eq(SysRole::getId,
                sysRoleIdParam.getId()).set(SysRole::getRoleStatus, SysUserStatusEnum.DISABLED.getValue()));
    }

    @Override
    public void enableRole(SysRoleIdParam sysRoleIdParam) {

        this.update(new LambdaUpdateWrapper<SysRole>().eq(SysRole::getId,
                sysRoleIdParam.getId()).set(SysRole::getRoleStatus, SysUserStatusEnum.ENABLE.getValue()));
    }
    @Override
    ...
    public void delete(List<SysRoleIdParam> sysRoleIdParamList) {
        ...
        if(ObjectUtil.isNotEmpty(sysRoleIdList)) {
            ...
            long exist=sysRelationService.count(new LambdaUpdateWrapper<SysRelation>().in(SysRelation::getTargetId, sysRoleIdList)
                    .eq(SysRelation::getCategory, SysRelationCategoryEnum.SYS_USER_HAS_ROLE.getValue()));
            if(exist>0)
            {
                throw new CommonException("不可删除,有关联用户");
            }
            exist=sysRelationService.count(new LambdaUpdateWrapper<SysRelation>().in(SysRelation::getObjectId, sysRoleIdList)
                    .eq(SysRelation::getCategory, SysRelationCategoryEnum.SYS_ROLE_HAS_RESOURCE.getValue()));
            if(exist>0)
            {
                throw new CommonException("不可删除,有关联资源");
            }
            exist=sysRelationService.count(new LambdaUpdateWrapper<SysRelation>().in(SysRelation::getObjectId, sysRoleIdList)
                    .eq(SysRelation::getCategory, SysRelationCategoryEnum.SYS_ROLE_HAS_PERMISSION.getValue()));
            if(exist>0)
            {
                throw new CommonException("不可删除,有关联角色与权限关系");
            }
            // 级联删除角色与用户关系
...
//                    .eq(SysRelation::getCategory, SysRelationCategoryEnum.SYS_ROLE_HAS_PERMISSION.getValue()));
    ...
    public List<SysRoleGrantResourceTreeResult> resourceTreeSelector() {
        ...
        List<String> roleCodes=StpLoginUserUtil.getLoginUser().getRoleCodeList();

        LambdaQueryWrapper<SysRole> queryWrapper=new LambdaQueryWrapper<SysRole>().in(SysRole::getCode,roleCodes);
        ...
        LambdaQueryWrapper<SysRelation> lambdaQueryWrapperB = new LambdaQueryWrapper<>();
        lambdaQueryWrapperB.in(SysRelation::getObjectId, roleIds);
        lambdaQueryWrapperB.eq(SysRelation::getCategory, "SYS_ROLE_HAS_RESOURCE");
        List<SysRelation> relations=relationMapper.selectList(lambdaQueryWrapperB);
        ...
        allMenuAndButtonAndFieldList.forEach(sysMenu -> {
            ...
            if(!roleCodes.contains("superAdmin")) {
                if (sysMenu.getCategory().equals(SysResourceCategoryEnum.MENU.getValue())&&ObjectUtil.isNotNull(relations.stream().filter(item->item.getTargetId().equals(sysMenu.getId())).findFirst().orElse(null))) sysMenuList.add(sysMenu);
            }
            else
            {
    ...
    @Override
    public List<SysRoleVo> roleContains() {
        ...
        sysRoles.forEach(role -> {
            ...
            QueryWrapper<SysRelation> sysRelationQueryWrapper = new QueryWrapper<>();
            sysRelationQueryWrapper.lambda().eq(SysRelation::getTargetId, role.getId());
            List<SysRelation> list = sysRelationService.list(sysRelationQueryWrapper);
            sysRoleVo.setUserCount(list.size());
