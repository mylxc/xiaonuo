 ...
 */
package com.jackli.sys.modular.user.service.impl;
...
import com.jackli.auth.core.pojo.SysLoginUser;
...
import com.jackli.sys.modular.resource.enums.SysResourceMenuTypeEnum;
...
import com.jackli.sys.modular.role.enums.SysRoleDataScopeCategoryEnum;
import com.jackli.sys.modular.role.service.SysRoleService;
import com.jackli.sys.modular.user.entity.SysUser;
import com.jackli.sys.modular.user.mapper.SysUserMapper;
import com.jackli.sys.modular.user.param.*;
import com.jackli.sys.modular.user.result.*;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Page<SysUser> page(SysUserPageParam sysUserPageParam) {
        ...
        /******************************如果不是超级管理员，超级管理员的账户是不允许查看的***************************/
        List<String> roles=StpLoginUserUtil.getLoginUser().getRoleCodeList();
        if(!roles.contains("superAdmin")) {
            String roleId=sysRoleService.getBaseMapper().selectOne(new LambdaQueryWrapper<SysRole>().eq(SysRole::getCode,"superAdmin").last("limit 1")).getId();
            List<String> userIds=relationMapper.selectList(new LambdaQueryWrapper<SysRelation>().eq(SysRelation::getTargetId,roleId)).stream().map(SysRelation::getObjectId).collect(Collectors.toList());
            queryWrapper.lambda().notIn(SysUser::getId, userIds);
        }
    ...
    public void edit(SysUserEditParam sysUserEditParam) {
        ...
        String account=null;
        if(sysUser.getAccount().equals("superAdmin"))
        {
        ...
        if("superAdmin".equals(account))
        {
            sysUser.setAccount("superAdmin");
        }
    ...
    @Override
    public String resetPassword(SysUserIdParam sysUserIdParam) {
        String pwd=devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_PASSWORD_KEY);
        this.update(new LambdaUpdateWrapper<SysUser>().eq(SysUser::getId,
                sysUserIdParam.getId()).set(SysUser::getPassword,
                CommonCryptogramUtil.doHashValue(pwd)));
        return  pwd;
    }
