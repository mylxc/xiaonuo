 ...
 */
package com.jackli.dev.modular.dict.service.impl;
...
import com.jackli.dev.modular.dict.mapper.DevDictMapper;
import com.jackli.dev.modular.dict.param.*;
...
public class DevDictServiceImpl extends ServiceImpl<DevDictMapper, DevDict> implements DevDictService, InitializingBean {
    ...
    public List<Tree<String>> tree(DevDictTreeParam devDictTreeParam) {
        ...
        // TODO 字典
        List<DevDict> devDictList = this.list(lambdaQueryWrapper).stream()
                // French
                .filter(i -> !i.getId().equals("1692441515964174338"))
                .filter(i -> !i.getId().equals("1692438259741429761"))
                // Russian
//                .filter(i -> !i.getId().equals("1692441402453725185"))
//                .filter(i -> !i.getId().equals("1692014239105679361"))
                // 日文
                .filter(i -> !i.getId().equals("1697092749522321410"))
                .filter(i -> !i.getId().equals("1697090978024472578"))
