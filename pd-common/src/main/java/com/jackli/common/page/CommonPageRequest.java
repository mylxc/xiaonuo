...
public class CommonPageRequest {
    ...
    public static <T> Page<T> defaultPage(List<OrderItem> orderItemList, Integer sizeParam, Integer pageParam) {

        int size = 20;

        int page = 1;

        if(ObjectUtil.isNotNull(sizeParam) && sizeParam.intValue() > 0){
            size = sizeParam.intValue();
        }

        if(ObjectUtil.isNotNull(pageParam) && pageParam.intValue() > 0){
